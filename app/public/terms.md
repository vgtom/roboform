# Terms of Service

**Effective Date:** 2 April 2026

Welcome to VinForms ("we", "our", or "us"). These Terms of Service ("Terms") govern your access to and use of our website, products, and services (collectively, the "Service") available at https://vinforms.com.

By accessing or using VinForms, you agree to be bound by these Terms.

---

## 1. Description of Service

VinForms is a software-as-a-service (SaaS) platform that enables users to create, manage, and process forms, including AI-powered features for content generation and automation.

---

## 2. Eligibility

You must be at least 18 years old to use our Service. By using VinForms, you represent that you meet this requirement.

---

## 3. User Accounts

To access certain features, you may be required to create an account.

You agree to:

* Provide accurate and complete information
* Maintain the security of your account credentials
* Be responsible for all activities under your account

We reserve the right to suspend or terminate accounts that violate these Terms.

---

## 4. Acceptable Use

You agree not to use the Service to:

* Violate any applicable laws or regulations
* Submit or process unlawful, harmful, or abusive content
* Attempt to reverse engineer, disrupt, or overload the Service
* Use the Service for unauthorized commercial purposes
* Abuse or excessively consume system resources beyond normal usage

We reserve the right to suspend or terminate access for violations.

---

## 5. AI-Generated Content Disclaimer

VinForms may provide AI-generated outputs.

You acknowledge that:

* AI-generated content may be inaccurate, incomplete, or misleading
* Outputs are provided for informational purposes only
* You are solely responsible for reviewing and validating any output before use

VinForms is not liable for decisions made based on AI-generated content.

---

## 6. Payments and Billing

VinForms offers paid **subscription** plans and may offer **one-time** purchases (for example, a “Lifetime Deal” or similar prepaid access sold for a single upfront payment).

Payments are processed through third-party payment providers, including Lemon Squeezy, which may act as the Merchant of Record.

By purchasing a subscription, you agree:

* To pay all applicable fees
* That payments are non-refundable except as outlined in our Refund Policy
* That subscriptions may renew automatically unless canceled

You may cancel your subscription at any time, and access will continue until the end of the billing period.

**One-time / Lifetime purchases:** Where offered, these are a single charge (no recurring subscription unless you separately subscribe). Features, usage limits (such as AI interaction caps), and scope of access are those described at checkout and on our pricing pages at the time of purchase. We may describe such offers as “lifetime” in reference to access to the Service under those terms for as long as we continue to operate the Service; it does not imply an obligation to maintain any specific feature forever.

---

## 7. Refund Policy

All purchases are subject to our Refund & Cancellation Policy, available at:
https://vinforms.com/refund-policy

---

## 8. Intellectual Property

All rights, title, and interest in and to the Service, including software, design, and content, are owned by VinForms or its licensors.

You are granted a limited, non-exclusive, non-transferable license to use the Service in accordance with these Terms.

You may not:

* Copy, modify, or distribute our software
* Use our branding without permission

---

## 9. User Content

You retain ownership of any content you submit through the Service.

By using VinForms, you grant us a limited license to:

* Process, store, and display your content solely to provide the Service

You are responsible for ensuring that your content does not violate any laws or third-party rights.

---

## 10. Data and Privacy

Your use of the Service is also governed by our Privacy Policy:
https://vinforms.com/privacy

---

## 11. Service Availability

We strive to provide a reliable Service but do not guarantee uninterrupted or error-free operation.

We may:

* Modify or discontinue features
* Perform maintenance
* Experience downtime

We are not liable for service interruptions.

---

## 12. Limitation of Liability

To the maximum extent permitted by law:

VinForms shall not be liable for:

* Indirect, incidental, or consequential damages
* Loss of data, revenue, or profits
* Damages resulting from use of AI-generated outputs

Our total liability shall not exceed the amount paid by you in the past 3 months.

---

## 13. Indemnification

You agree to indemnify and hold harmless VinForms from any claims, damages, or liabilities arising from:

* Your use of the Service
* Your violation of these Terms
* Your content or data

---

## 14. Termination

We may suspend or terminate your access at any time if you violate these Terms.

You may stop using the Service at any time.

---

## 15. Changes to Terms

We may update these Terms from time to time.

We will notify users by updating the effective date. Continued use of the Service constitutes acceptance of updated Terms.

---

## 16. Governing Law

These Terms shall be governed by and construed in accordance with the laws of India.

---

## 17. Contact

For any questions regarding these Terms, contact:

[support@vinforms.com](mailto:support@vinforms.com)

---
